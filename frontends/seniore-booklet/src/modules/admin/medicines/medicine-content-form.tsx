...
export const medicineSchema = z.object({
  ...
  description: z.string({ message: "Description is required" }).min(5, "Description must contain at least 5 character(s)").max(400, { message: "Description must only contains 400 characters" }).trim(),
  brandName: z.string().min(3, "Brand Name must contain at least 3 character(s)").max(100, { message: "Brand Name must only contains 100 characters" }).trim(),
  genericName: z.string().min(3, "Generic Name must contain at least 3 character(s)").max(100, { message: "Generic Name must only contains 100 characters" }).trim(),
  strength: z.string().min(3, "Strength must contain at least 3 character(s)").max(100, { message: "Strength must only contains 100 characters" }).trim(),
  dosageForm: z.string().min(3, "Dosage Form must contain at least 3 character(s)").max(100, { message: "Dosage Form must only contains 100 characters" }).trim(),
  medicineImageFile: z.instanceof(File)
...
const MedicineContentForm = ({ medicine = {}, onOpenChange, open }: MedicineContentFormProps) => {
  ...
  const form = useForm<MedicineFormValues>({
    ...
    mode: "onTouched",
    defaultValues: isEditingMode && medicine
    ? { ...otherData, }
    : defaultValues,
});
  ...
  const onSubmit: SubmitHandler<MedicineFormValues | any> = async (data: MedicineFormValues) => {
    try {
      const medicineData = {
        ...data,
        isActive: true,
    ...
    <Sheet onOpenChange={setSheetOpen} open={open}>
      <SheetContent className=" p-0 flex flex-col h-full md:max-w-[40rem]">
        ...
        <Button
          ...
          >
            {isEditingMode
              ? isUpdatingMedicine ? "Updating Medicine.. " : "Update Medicine" 
              : isAddingMedicine
                ? "Creating Medicine..."
                : "Create Medicine"}
          </Button>
