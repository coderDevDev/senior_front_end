...
export default {
  ...
  updatePharmacy: async (pharmacyId: string, data: Partial<PharmacyFormValues>) => {
    try {
      const formData = new FormData();
  ...
  archivePharmacy: async (payload: any) => {
    try {
      const response = await axios({
        method: "PUT",
        url: `${import.meta.env.VITE_SERVER_URL}/api/v1/pharmacy/archive_pharmacy`,
        data: payload
      });
      return response.data;
    } catch (err) {
  ...
  unarchivePharmacy: async (payload: any) => {
    try {
      const response = await axios({
        method: "PUT",
        url: `${import.meta.env.VITE_SERVER_URL}/api/v1/pharmacy/unarchive_pharmacy`,
        data: payload
      });
      return response.data;
    } catch (err) {
