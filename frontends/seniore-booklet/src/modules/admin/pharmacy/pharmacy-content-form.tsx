...
export const pharmacySchema = z.object({
  ...
  address: z
    ...
    .min(5, 'Address must contain at least 5 character(s)')
    .max(200)
    .trim(),
...
const PharmacyContentForm = ({
  pharmacy = {},
  onOpenChange,
  open
}: PharmacyContentFormProps) => {
  ...
  const form = useForm<PharmacyFormValues>({
    ...
    mode: 'onTouched',
    defaultValues: isEditingMode && pharmacy ? { ...otherData } : defaultValues
  });
  ...
  useEffect(() => {
    if (open) {
      form.reset(isEditingMode && pharmacy ? { ...otherData } : defaultValues);
    } else {
  ...
  const onSubmit: SubmitHandler<PharmacyFormValues | any> = async (
    data: PharmacyFormValues
  ) => {
    try {
      const pharmacyData = {
        ...data,
