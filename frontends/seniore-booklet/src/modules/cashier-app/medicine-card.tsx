    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
      <div className="p-4">
        <div className="flex items-start">
          <div className={`flex-shrink-0 w-16 h-16 rounded-md flex items-center justify-center mr-4 ${getCategoryColor(medicine.brandName)}`}>
            <img 
          ...
          <div className="flex-grow">
            <div className="flex justify-between items-start">
              ...
              <span className="font-medium text-gray-800">₱{medicine.unitPrice?.toFixed(2)}</span>
            </div>
            <p className="text-gray-600 text-sm mb-2">{medicine.description}</p>
            <div className="flex flex-wrap gap-2">
              <span className={`text-xs px-2 py-1 rounded-full ${getCategoryColor(medicine.brandName)}`}>
                {medicine.brandName}
              </span>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="flex items-center bg-slate-100 rounded-full overflow-hidden">
            <button 
              ...
              disabled={!quantity}
            >
              <MinusCircle className={`w-5 h-5 ${quantity ? 'text-gray-700' : 'text-gray-400'}`} />
            </button>
