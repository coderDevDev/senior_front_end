...
export class FingerprintService {
   ...
   */
  async registerFingerprint(data: FingerprintRegistrationData): Promise<ApiResponse<{ id: string }>> {
    try {
      // Validate the input
   ...
   */
  async verifyFingerprint(userId: string, templateData: string): Promise<ApiResponse<{ matched: boolean; score?: number; }>> {
    try {
   ...
   */
  async getUserFingerprints(userId: string): Promise<ApiResponse<Array<{
    id: string;
    fingerPosition: FingerPosition;
    quality: number;
    createdAt: string;
   ...
   */
  private handleError(err: unknown, defaultMessage: string): ApiResponse<never> {
